...
from src.model.discriminator.lpips import LPIPS
from src.model.discriminator.discriminator_patchgan import NLayerDiscriminator as PatchGANDiscriminator
from src.model.discriminator.discriminator_stylegan import Discriminator as StyleGANDiscriminator
from src.model.discriminator.discriminator_dino import DinoDisc as DINODiscriminator
from src.model.discriminator.diffaug import DiffAug
import wandb
...
class VQLoss(nn.Module):
    ...
    def forward(self, results, sem_loss, detail_loss, dependency_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, log_data_func=None, log_every=100, fade_blur_schedule=0):
        # generator update
        if optimizer_idx == 0:
            # reconstruction loss
            ...
            loss = self.rec_weight * rec_loss + \
                ...
                results['mean_vq_loss'] + results['mean_commit_loss'] + results['mean_entropy'] + sem_loss + detail_loss + dependency_loss
            ...
            if global_step % log_every == 0:
                ...
                log_data_func({
                    ...
                    "vq_loss": results['mean_vq_loss'],
                    "commit_loss": results['mean_commit_loss'],
                    "entropy_loss":results['mean_entropy'],
                    "codebook_usage": results['codebook_usage'],
                    "codebook_usage_var": results['codebook_usage_var'],

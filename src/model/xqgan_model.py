...
from src.model.vqgan.cliploss import ClipLoss
...
from src.model.quantifier.causal_quantize import CQ
from src.model.quantifier.quant import VectorQuantizer2, VectorQuantizer
from src.model.quantifier.lookup_free_quantize import LFQ
from src.model.quantifier.fsq import RotFSQ
from src.model.quantifier.simrq import SimRQ
from src.model.dino_enc.dinov2 import DINOv2Encoder, DINOv2Decoder
from src.dataset.normalize import Denormalize
from src.dataset.normalize import Normalize as ImgNormalize
...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs,):
        ...
        if config.enc_type == 'cnn':
            ...
            self.encoder = DINOv2Encoder(
                ...
                model_kwargs={'img_size': config.image_size, 'patch_size': 16, 'drop_path_rate': 0.1},
                pretrained=True,
                tuning_method='full',
                tuning_kwargs={'r':8},
        ...
        elif config.vq_type == 'simrq':
            self.quantize = SimRQ(config.codebook_size, config.codebook_embed_dim)
        ...
        elif config.vq_type == 'rotfsq':
            self.quantize = RotFSQ(config.codebook_size, config.codebook_embed_dim)
        elif not config.lfq:
    ...
    def forward(self, input, epoch):
        ...
        dependency_loss = 0.0
        quant, codebook_usage, codebook_usage_var, mean_vq_loss, mean_commit_loss, mean_entropy = self.quantize.forward(h, ret_usages=True, dropout=dropout_rand)
        ...
        return {'recons_imgs': dec, 
                'mean_vq_loss': mean_vq_loss,
                'mean_commit_loss': mean_commit_loss,
                'mean_entropy': mean_entropy,
                'sem_loss': sem_loss,
                'detail_loss': detail_loss,
                'dependency_loss': dependency_loss,
                'codebook_usage': codebook_usage,
                'codebook_usage_var': codebook_usage_var
                }
    ...
    def img_to_reconstructed_img(self, x, last_one=True,) -> List[torch.Tensor]:
        ...
        if self.product_quant > 1:
            ...
            if len(self.v_patch_nums) == 1:
                f_hats_list = [self.quantizes[i].f_to_idxBl_or_fhat(f, v_patch_nums=self.v_patch_nums, to_fhat=True) for i, f in enumerate(f_list)]
            else:

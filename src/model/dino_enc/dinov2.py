...
class DINOv2Encoder(nn.Module):
    def __init__(self, in_channels=3, num_latent_tokens=32, use_attn_mask=False,
        ...
        # # 加载模型  
        if pretrained:  
            # 使用 Hugging Face hub 下载模型权重到本地  
            checkpoint_path = hf_hub_download(  
                repo_id="timm/vit_base_patch14_dinov2.lvd142m",  # 替换为对应的 Hugging Face 仓库  
                filename="model.safetensors",  # 使用 Hugging Face 提供的权重文件  
                cache_dir="/nfs_beijing/kubeflow-user/zhangyang_2024/workspace/BSQ",  # 指定缓存目录，路径可变  
                force_filename="dinov2_vitb14_pretrain.pth"  # 可指定本地保存的文件名  
            )  
        else:  
            checkpoint_path = None  # 如果模型不需要预训练权重 

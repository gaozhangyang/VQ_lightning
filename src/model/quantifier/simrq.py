...
class SimRQ(nn.Module):
    def __init__(self, vocab_size, z_channels=32):
        super(SimRQ, self).__init__()
        ...
        for i in range(len(levels)):
            thresholds.append(F.pad(torch.linspace(-1, 1, levels[i]),
                                    (0, max(self._levels)-1), mode='constant', value=0.0).unsqueeze(0))
        thresholds = torch.stack(thresholds, dim=0)        
        self.register_buffer("threshold", thresholds)
